...
def statistics_dictionary(observation, model, var):
    """ This script calls statsBryant (both non-directioanl and directional versions) function and creates stats_dict.
     ...
     Args:
        ...
        'observation':  vector of observationervations
    ...
    if isinstance(observation, np.ma.masked_array) and ~observation.mask.any():
        observation = np.array(observation)
    if isinstance(model, np.ma.masked_array) and ~model.mask.any():
        model = np.array(model)

    if 'D' in var:  # check if var is directional, to call circular stats function
        stats_dict = statsBryantCirc(observation, model)
    else:  # check if var is non-directional
        stats_dict = statsBryant(observation, model)

    stats_dict['m_mean'] = np.nanmean(model)  # compute model mean
    stats_dict['o_mean'] = np.nanmean(observation)  # compute observation mean
...
def obs_v_mod_subplot(ofname,obs_dict, model_dict):
    """ This script compares model's time series to observational data. Each row is a different station;
    whereas each column is a different variable (for example: Hs, Tm, Dir, etc...).

            Args:
                ofname: output file name
                obs_dict: dictionary with stations observations
                model_dict: dictionary with model results at station locations"""
    ...
    ## save figure
    f.tight_layout()
    plt.savefig(ofname, bbox_inches='tight', dpi=80)
    plt.close()


def obs_v_mod_stats_subplot(ofname,stats_dict):
    """ This script compares bar charts of stats_dict.

        Args:
            ofname: output file name
            stats_dict: has keys for each model compared to field data stations. Each variable has 14 stats keys:
    ...
    # create model, station, y-label and x-label lists
    y_labels = []
    if not y_labels:
        model = list(stats_dict.keys())  # model list
        station_list = list(stats_dict[model[0]].keys())  # field station list
        y_labels = list(stats_dict[model[0]][station_list[0]].keys())  # y label list (variables)
        x_labels = list(stats_dict[model[0]][station_list[0]][y_labels[0]].keys())  # x label list (stats)

    varlen = len(y_labels)  # number of variables (figure column)
    obslen = len(station_list)  # number of field stations (figure row)
    statlen = len(x_labels)  # number of stats variables

    # figure specs
    width = 14  # figure width
    height = 8  # figure height
    colors = ['r', 'b', 'g', 'c', 'm', 'y']  # bar colors for models
    ind = np.arange(statlen)  # x axis tick array
    wid = 1 / (len(model) + 1)  # bar width

    # start figure
    f, ax = plt.subplots(obslen, varlen, sharex=True, figsize=(width, height))
    for ii, mod in enumerate(stats_dict.keys()):
        for jj, obs in enumerate(stats_dict[mod].keys()):
            for kk, var in enumerate(stats_dict[mod][obs].keys()):
                for ll, stat in enumerate(stats_dict[mod][obs][var].keys()):
                    ax[jj, kk].bar(ind[ll] + wid * ii, stats_dict[mod][obs][var][stat], color=colors[ii],
                                   align='center', width=wid)

    # set x labels and field station labels per row
    for i in range(obslen):
        for j in range(varlen):
            ax[i, j].grid(which='major', axis='y', linestyle='--') # set grid
            ax[i, j].set_xticks(ind + wid)  # set x tick locations
            ax[i, j].set_xticklabels(labels=x_labels, rotation=45, horizontalalignment='right')  # set x tick labels
            ax[i, j].text(0.05, 0.90, station_list[i],  # set field station label per row
                          transform=ax[i, j].transAxes,
                          horizontalalignment='left',
                          verticalalignment='top',
                          weight="bold",
    ...
    # set y labels per column
    for kk, label in enumerate(y_labels):
        x_coord = 1 / varlen * kk
        plt.figtext(x_coord, 0.5, label, fontsize=16, rotation='vertical')

    # set legend at top of plot
    leg = f.legend(labels=model,
                   loc='upper center',
                   ncol=len(model),
                   bbox_to_anchor=(0.5, 1.05),
                   fontsize=16)
